...
if (isset($_POST['idVendedor'])){
	$idVendedor=htmlentities(trim($_POST['idVendedor']));
	//$idVendedor=mysql_real_escape_string($idVendedor);
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo idVendedor no es correcto</p>";
}
...
if($validacion){
	$resultado=$Ovendedor->actualizar($nombre,$estatus, $idVendedor);
	if($resultado=="exito"){
