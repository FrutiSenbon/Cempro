...
if (isset($_POST['idCatalogo'])){
	$idCatalogo=htmlentities(trim($_POST['idCatalogo']));
	//$idCatalogo=mysql_real_escape_string($idCatalogo);
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo idCatalogo no es correcto</p>";
}
...
if($validacion){
	$resultado=$Oproducto->actualizar($nombre,$uniMedEnt,$uniMedSal,$equivalencia,$estatus, $idCatalogo);
	if($resultado=="exito"){
