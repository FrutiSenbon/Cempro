...
if (isset($_POST['idAditivos'])){
	$idAditivos=htmlentities(trim($_POST['idAditivos']));
	//$idAditivos=mysql_real_escape_string($idAditivos);
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo idAditivos no es correcto</p>";
}
...
if($validacion){
	$resultado=$Oaditivo->actualizar($nombre,$estatus, $idAditivos);
	if($resultado=="exito"){
