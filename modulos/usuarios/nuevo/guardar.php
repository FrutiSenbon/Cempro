...
if (isset($_FILES['foto']['name'])){
	...
	$extencionfoto=pathinfo($_FILES['foto']['name'], PATHINFO_EXTENSION);
	$foto=basename($_FILES['foto']['name'],".".$extencionfoto)."_".generarClave(5).".".$extencionfoto;
...
if (isset($_POST['idregistrorelacionado'])){
	$idregistrorelacionado=htmlentities(trim($_POST['idregistrorelacionado']));
	//$idregistrorelacionado=mysql_real_escape_string($idregistrorelacionado);
	
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo idregistrorelacionado no es correcto</p>";
}

if (isset($_POST['tablarelacionada'])){
	$tablarelacionada=htmlentities(trim($_POST['tablarelacionada']));
	//$tablarelacionada=mysql_real_escape_string($tablarelacionada);
	
}else{
	$validacion=false;
	$mensaje=$mensaje."<p>El campo tablarelacionada no es correcto</p>";
}
...
if($validacion){
	$resultado=$Ousuario->guardar($nombre,$email,$foto,$usuario,$contrasena,$estado,$idperfil,$empresa,$controlaracceso,$horainicio,$horafin,$idregistrorelacionado,$tablarelacionada,$bitacora);
	if($resultado=="exito"){
		/*CARGAR ARCHIVOS*/
		$mensajeArchivo="";
